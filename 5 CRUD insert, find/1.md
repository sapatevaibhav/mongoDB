...
There are teo types of insertion `insertOne()` and `insertMany()`

for `insertMany()` we have to pass documents in array.

MongoDB autometically assigns unique `_id` to each document if not specified.
